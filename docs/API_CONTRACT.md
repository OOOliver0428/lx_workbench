...
- `POST /api/v1/work-records/quick-create` 一站式创建工作记录：可同时选择或新建项目/部门工作/
  ...
  项目或部门工作来源上。
- 工作记录创建、快速创建和编辑支持 `time_blocks: [{"start": 540, "end": 630}]`。起止值是
  当天 00:00 起的分钟数，`end` 不含，范围为 0–1440 且必须是 30 的倍数；区间不得重叠，
  相邻区间必须先合并，单次最多 24 段。响应始终返回按开始时间排序的 `time_blocks`，历史记录
  返回空数组。
- 创建或编辑提交非空 `time_blocks` 时，服务端以所有区间长度之和覆盖 `minutes`，客户端传入的
  汇总值不作为事实来源。不提交该字段时保持旧客户端的 `minutes` 行为；编辑传 `null` 或省略
  表示不改区间，传 `[]` 表示清空区间明细并保留当前汇总分钟数。
...
- 后端只按当前用户的生效权限构造项目、任务、工作记录上下文；缺少相应查看权限时不会把该类
  数据或名称发送给模型。上下文中的负责人与关联对象以名称互指，并带时间锚点；各分段除数量
  上限外还受总字符预算约束，被截断的分段会写入 `truncated_sections`。
- `POST /api/v1/ai/chat` 按用户持久化对话历史并注入后续请求；历史受消息数与字符双预算
  裁剪，最旧内容先丢弃。`GET /api/v1/ai/chat/history` 返回当前用户的历史（要求 `ai.use`），
  `DELETE /api/v1/ai/chat/history` 清空并写入审计。
...
- 更新日志接口支持 `category: "release"`，`title` 填版本号（例如 `0.3.1` 或 `v0.3.1`），
...
- `POST /api/v1/ai/chat` 的业务上下文由后端根据当前用户的生效权限生成，浏览器不能提交或扩大
  ...
  对话历史按用户持久化、跨会话保留，并受轮数与字符预算裁剪；浏览器可通过
