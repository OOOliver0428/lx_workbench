...
> 适用版本：**0.5.0** · 最后复核：2026-09-22 · 正式发布分支：`main`

正式部署前须确认 `main` 的 CI 通过并完成版本标签发布。
从 0.4.1 升级新增日志排序迁移；从 0.3.0 升级还包含管理范围、版本日志和周成员名单迁移，目标为 `f8a1b2c3d4e5`。维护窗口先备份再升级；回滚限制见
[0.5.0 更新日志](../CHANGELOG.md#050---2026-09-22)。
...
cd lx_workbench
git checkout v0.5.0   # 正式发布该标签后，锁定到已评审的版本
...
- [ ] 正式生产开放前已单独评审并提高密码长度和强度策略；0.5.0 试运行阶段仍保留现有策略。
