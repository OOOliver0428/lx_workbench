...
> 适用版本：**0.4.0** · 最后复核：2026-09-11 · 正式发布分支：`main`

0.4.0 当前在 `dev` 集成；正式部署前须完成 `main` 合并与版本标签发布。
从 0.3.0 升级包含管理范围和版本日志两项数据库迁移，维护窗口先备份再升级；回滚限制见
[0.4.0 更新日志](../CHANGELOG.md#040---2026-09-11)。
...
cd lx_workbench
git checkout v0.4.0   # 正式发布该标签后，锁定到已评审的版本
...
- [ ] 正式生产开放前已单独评审并提高密码长度和强度策略；0.4.0 试运行阶段仍保留现有策略。
