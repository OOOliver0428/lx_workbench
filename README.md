...
> 当前版本：**0.4.1** · 当前集成分支：`dev` · 正式发布分支：`main`
...
- 系统维护账号拥有全部权限但不进入业务用户候选目录；个人周报始终只对作者本人可见。
